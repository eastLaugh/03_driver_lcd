...
extern float humidity;//extern可以在一个文件中引用另一个文件中定义的变量或者函数
...
static void onenet_upload_entry(void *parameter)
{
    while (1)
    {
        ...
        if (onenet_mqtt_upload_digit("temperature",  (int)temperature) < 0)
        {
            LOG_E("upload has an error, stop uploading");
            break;
        }
        rt_thread_delay(rt_tick_from_millisecond(5 * 1000));
        if (onenet_mqtt_upload_digit("humidity", (int)humidity) < 0)
        {
        LOG_E("upload has an error, stop uploading");
        break;
        }
        ...
        LOG_D("buffer : {\"humidity\":%d}", (char)humidity);
        LOG_D("buffer : {\"temperature\":%d}", (char)temperature);
        }
        rt_thread_delay(rt_tick_from_millisecond(5 * 1000));
    }
}
